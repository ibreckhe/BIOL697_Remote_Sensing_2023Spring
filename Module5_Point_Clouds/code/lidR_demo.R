...
library(future)
library(rgl)
setwd("~/code/BIOL697_Remote_Sensing_2023Spring/")
...
plot(sfm_norm,color="GCC",bg="white",axes=TRUE,legend=TRUE,breaks='quantile')
...
crown_metrics <- delineate_crowns(sfm_seg, 
                                   func = ~custom_tree_metrics(z = Z, 
                                                               gcc = GCC))
crown_metrics_sf <- st_as_sf(crown_metrics)
plot(crown_metrics_sf['GCC_mean']) 
st_write(crown_metrics_sf,dsn="./Module5_Point_Clouds/data/crown_polygons.gpkg",delete_dsn=TRUE)
